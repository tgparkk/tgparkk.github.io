...
## 1.2.1 영상 표현 방법
**그레이스케일(grayscale) : 색상 정보 없이 오직 밝기 정보만으로 구성된 영상을 의미. 영상 처리에서 주로 사용하는 영상을 의미하며 흑백 사진처럼 검정색, 회색, 흰색으로 구성되어 있다.**

**트루컬러(true-color) : 이와는 반대로 색상정보를 가지고 있어서 다양한 색상을 표현한다.**
