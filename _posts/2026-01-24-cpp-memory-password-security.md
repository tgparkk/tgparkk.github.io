...
| 항목 | 취약한 방법 | 안전한 방법 |
|------|------------|------------|
| **저장** | `std::string` | `SecureString` 또는 `EncryptedString` |
| **메모리 정리** | `clear()`, `= ""` | `SecureZeroMemory()` |
| **비교** | `==` 연산자 | 상수 시간 비교 함수 |
| **입력** | `std::cin` | 에코 비활성화 + 버퍼 제로화 |
| **전송** | 평문 | TLS/SSL 암호화 |

### 추가 권장사항

1. **가능한 빨리 정리**: 비밀번호는 사용 직후 즉시 메모리에서 제거
2. **복사 최소화**: 비밀번호 복사를 최소화하여 메모리에 남는 흔적 감소
3. **디버그 빌드 주의**: 디버그 모드에서는 최적화가 적용되지 않아 더 많은 정보가 노출될 수 있음
4. **메모리 잠금**: 중요 데이터는 `VirtualLock()`으로 스왑 파일로 내려가는 것을 방지
5. **정기 감사**: 메모리 분석 도구로 정기적으로 보안 점검

---

## 마무리

`std::string`에 비밀번호를 저장하는 것은 생각보다 큰 보안 위험을 초래합니다. 메모리 덤프 한 번이면 모든 민감한 정보가 노출될 수 있습니다.

이 글에서 소개한 `SecureZeroMemory`와 DPAPI를 활용하면 메모리에서 비밀번호가 유출되는 위험을 크게 줄일 수 있습니다. 보안은 작은 습관에서 시작됩니다.
