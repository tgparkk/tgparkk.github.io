...
# 주식 단타 전략 소개 – 한국인 독자를 위한 짧은 매매

과거에도 비슷한 전략 글을 썼지만, 이번에는 **단타**에 초점을 두고, **두 가지 방식**으로 정리했습니다. 하나는 웹에서 단타·스캘핑 전략을 탐색한 것이고, 다른 하나는 각 전략이 **한국투자증권(KIS) API**로 구현 가능한지 확인한 것입니다.
...
- **왜 매수했는지**: 가격이 이동평균선 위로 크게 올랐다가 **하락하기 시작한 구간**에서, 평균선까지 **반등할 것**을 기대하며 매수했습니다. (평균 회귀는 "너무 떨어지면 반등, 너무 오르면 하락"을 노리는 전략입니다.)
