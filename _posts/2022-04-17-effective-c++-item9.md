...
## summary
- 파생 클래스 객체가 생성될 때 그 객체의 기본 클래스 부분이 먼저 호출된다.
- 기본 클래스의 생성자가 호출될 동안에는, 가상 함수는 절대로 파생 클래스 쪽으로 내려가지 않습니다.
- 소멸자가 호출될때에도 동일합니다.
- 생성 중이거나 소멸 중인 객체에 대해 생성자나 소멸자에서 가상 함수를 호출하는 코드를 철저히 솎아 내자
- 이런한 문제를 해결하기 위해서는, 가상 함수를 기본 클래스의 비가상 멤버 함수로 바꾼후, 파생 클래스의 생성자로 하여금 필요한 값을 기본 생성자의 매개변수로 넘기는 겁니다.
- 또한, 매개변수로 넘길때 정적 함수을 이용해 넘긴다면 미초기화된 데이터 멤버를 건드릴 위험도 없습니다.
